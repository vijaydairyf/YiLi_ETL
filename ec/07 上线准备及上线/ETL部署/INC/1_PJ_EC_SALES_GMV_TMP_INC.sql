...
a.pro_categ_name,
a.pro_brand_name,
case when a.pro_grade='无' or a.pro_grade='未定义'  or a.pro_grade ='缺省' or  a.pro_grade ='其他' then '空' else a.pro_grade end pro_grade,
a.site_wid,
...
3 as bg_wid,
nvl(t5.platform_name,'其他') as platform_name,
nvl(t5.sale_area_name,'其他') as sale_area_name,
-1 as dealer_wid,
nvl(t4.erp_id,'其他') as erp_id,
t2.pro_id as pro_id,
nvl(t4.pro_categ_name,'其他') as pro_categ_name,
nvl(t4.pro_brand_name,'其他') as pro_brand_name,
nvl(t4.pro_grade,'其他') as pro_grade,
-1 as site_wid,
...
from
(select
nvl(if(trim(t.pro_id)='',null,trim(t.pro_id)),'其他')   as pro_id,
...
t.stats_dt,
nvl(if(trim(t.store_nm)='',null,trim(t.store_nm)),'其他')  as sale_area_name
from datalake_md.fact_ec_chan_sale_dtl t
where t.stats_dt is not null 
and t.stats_dt >=regexp_replace(trunc(add_months(date_sub(current_date,1),-1),'MM'),'-','') 
and t.stats_dt <= regexp_replace(last_day(date_sub(current_date,1)),'-','')
and cast(nvl(if(trim(t.deal_amt)='',null,trim(t.deal_amt)),0)  as bigint) <>0
) t2
...
from datalake_md.fact_ec_budget_plan where src_identity='GMV' and year_month >=substr(regexp_replace(trunc(add_months(date_sub(current_date,1),-1),'MM'),'-',''),1,6) and year_month <=regexp_replace(substr(date_sub(current_date,1),1,7),'-','')) b
...
where date_>=regexp_replace(substr(add_months(date_sub(current_date,1),-1),1,7),'-','')
...
where add_months(concat_ws('-',substr(mago.sales_dt_wid,1,4),substr(mago.sales_dt_wid,5,2),substr(mago.sales_dt_wid,7,2)),1)<current_date()
and mago.date_ in (regexp_replace(substr(add_months(date_sub(current_date,1),-2),1,7),'-',''),regexp_replace(substr(add_months(date_sub(current_date,1),-1),1,7),'-',''))
...
where add_months(concat_ws('-',substr(tmago.sales_dt_wid,1,4),substr(tmago.sales_dt_wid,5,2),substr(tmago.sales_dt_wid,7,2)),3)<current_date()
and tmago.date_ in (regexp_replace(substr(add_months(date_sub(current_date,1),-4),1,7),'-',''),regexp_replace(substr(add_months(date_sub(current_date,1),-3),1,7),'-',''))
...
where add_months(concat_ws('-',substr(yago.sales_dt_wid,1,4),substr(yago.sales_dt_wid,5,2),substr(yago.sales_dt_wid,7,2)),12)<current_date()
and yago.date_ in (regexp_replace(substr(add_months(date_sub(current_date,1),-12),1,7),'-',''),regexp_replace(substr(add_months(date_sub(current_date,1),-13),1,7),'-',''))
) t
group by
t.sales_dt_wid,
t.bg_wid,

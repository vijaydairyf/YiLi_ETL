...
where t.stats_dt is not null and t.stats_dt >=regexp_replace(trunc(add_months(date_sub(current_date,1),-1),'MM'),'-','') and t.stats_dt <= regexp_replace(last_day(date_sub(current_date,1)),'-','')
) t4
where flag =1
) t2

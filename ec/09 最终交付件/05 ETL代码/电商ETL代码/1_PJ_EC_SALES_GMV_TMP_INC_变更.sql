...
t2.sales_amt_before as sales_amt_before,
0 sales_amt_after,
...
cast(nvl(if(trim(t.deal_amt)='',null,trim(t.deal_amt)),0)  as decimal(38,10)) as sales_amt_before,
t.stats_dt,
...
left join (select  platform_name,sale_area_name as sale_area_name1 ,final_name as sale_area_name from bigdata_dw.w_ec_area_d_tmp where platform_type = 'GMV') t5 on t2.sale_area_name = t5.sale_area_name1 where nvl(t4.pro_categ_name,'其他') <> '剔除项') a
group by 
a.sales_dt_wid,
a.bg_wid,
a.platform_name,
...
--不再读取20190901以后品类为婴儿粉、酸奶的销售数据
insert overwrite table bigdata_dm.pj_ec_sales_gmv_tmp_tmp partition(date_)
select * from bigdata_dm.pj_ec_sales_gmv_tmp
where sales_dt_wid <= 20190831
union all 
select * from bigdata_dm.pj_ec_sales_gmv_tmp
where sales_dt_wid >= 20190901 and pro_categ_name <> '婴儿粉' and pro_categ_name <> '酸奶';
...
from bigdata_dm.pj_ec_sales_gmv_tmp_tmp
where date_>='201801'
...
0 yago_budget_amt_before,
0 yago_budget_amt_after
from bigdata_dm.pj_ec_sales_gmv_tmp_tmp mago
...
0 yago_budget_amt_before,
0 yago_budget_amt_after
from bigdata_dm.pj_ec_sales_gmv_tmp_tmp tmago
...
from bigdata_dm.pj_ec_sales_gmv_tmp_tmp yago

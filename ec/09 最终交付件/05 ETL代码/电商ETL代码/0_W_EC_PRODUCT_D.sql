...
nvl(if(trim(import_no)='',null,trim(import_no)),'其他') as sku_id,
nvl(if(trim(pro_no)='',null,trim(pro_no)),'其他') as erp_id,
trim(pro_name)  as erp_name,
trim(pro_specfc) as erp_unit,
trim(import_full)  as platform_name,
trim(import_specfc) as platform_unit,
nvl(if(trim(pro_name_complt)='',null,trim(pro_name_complt)),'其他') as final_name,
'其他' as pro_categ_cd,
...
where nvl(if(trim(import_no)='',null,trim(import_no)),'其他') <>'其他'
union all
select
t.row_wid,
t.seq_num,
...
platform_name,
platform_unit,
nvl(final_name,'其他') as final_name,
pro_categ_cd,
nvl(pro_categ_name,'其他') as pro_categ_name,
pro_brand_cd,
nvl(pro_brand_name,'其他') as pro_brand_name,
nvl(pro_grade,'其他') as pro_grade,
proc_coef,
if_comb_flag,
skuid_seq,
attr1,
...
t.erp_id,
nvl(t.erp_name,'其他') as erp_name,
t.erp_unit,
nvl(t.pro_categ_name,'其他') as pro_categ_name,
nvl(t.pro_brand_name,'其他') as pro_brand_name,
nvl(t.pro_grade,'其他') as pro_grade,
t.w_insert_dt
from (
select
nvl(if(trim(pro_no)='',null,trim(pro_no)),'其他') as erp_id,
nvl(trim(pro_specfc),'其他') as erp_unit,
nvl(trim(pro_name_complt),'其他')  as erp_name,
nvl(if(trim(pro_categ)='',null,trim(pro_categ)),'其他') as pro_categ_name,
nvl(if(trim(pro_brand_name)='',null,trim(pro_brand_name)),'其他') as pro_brand_name,
nvl(if(trim(pro_grade)='',null,trim(pro_grade)),'其他') as pro_grade,
row_number() over (partition by nvl(if(trim(pro_no)='',null,trim(pro_no)),'其他') order by trim(attr2)) as skuid_seq,
from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as w_insert_dt
from datalake_md.d_dm_ec_pro
where nvl(if(trim(pro_no)='',null,trim(pro_no)),'其他') <>'其他') t
...
'其他' as pro_categ_name,
nvl(t2.product_child_brand,'其他') as pro_brand_name,
nvl(t2.product_grade,'其他') as pro_grade,
from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss')  as w_insert_dt

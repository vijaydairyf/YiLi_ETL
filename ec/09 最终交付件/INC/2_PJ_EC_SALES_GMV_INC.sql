...
yago_budget_amt_before,
yago_budget_amt_after
from (select row_wid day_id from bigdata_dw.w_date_d where dt_type='D' and row_wid>=cast(regexp_replace(trunc(add_months(date_sub(current_date,1),-1),'MM'),'-','') as bigint) and row_wid<=cast(regexp_replace(last_day(date_sub(current_date,1)),'-','') as bigint)) b
left join (select * from bigdata_dm.pj_ec_sales_gmv_ago where date_ >=regexp_replace(substr(add_months(date_sub(current_date,1),-1),1,7),'-','')) a

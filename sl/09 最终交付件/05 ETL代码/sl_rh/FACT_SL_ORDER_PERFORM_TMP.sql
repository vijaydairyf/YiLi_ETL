...
select t1.datasource_id,
  ...
  from (
        ...
        select 'DMS' datasource_id,
         ...
         where tt1.source = '手工'
         group by tt1.erpid, tt1.erpno, tt1.product_no, tt2.organization_id) t1 --1207955 1226781
...
WHERE t2.bg_name in('奶粉事业部','酸奶事业部','冷饮事业部','健康饮品事业部')  and substr(t2.booked_date, 1, 10) >= date_sub(current_date,8);

insert into table bigdata_dw.fact_sl_order_perform_tmp
select t1.datasource_id,
       t1.integration_id,
       t1.header_id,
       t1.order_number,
  ...
  left join (select header_id,
   ...
   and t1.cpbh = t4.item_no
   and t1.wh_code = t4.wh_code
 where t2.bg_name = '液态奶事业部' and substr(t2.booked_date, 1, 10) >= date_sub(current_date,8);

...
in_store_t,      --有货量（吨
out_store_t,     --缺货量（吨）
out_store_qty,     --缺货品项个数
wh_plan_sale_t,  --分仓计划销量汇总
ly_sku_qty,      --同期有货品项个数
ly_plan_sku_qty,   --同期计划品项
ly_inventory_t,    --同期库存量（吨）
ly_plan_sales_t,   --同期计划销量（吨）
ly_in_store_t,     --同期有货量（吨）
b.target_in_store,   --有货率目标值(%)
b.target_in_store_sku, --有货率品项目标值(%)
b.target_in_store_t,   --有货率货量目标值(%)
...
select
 ...
 ( --同期
  select substr(day_wid, 1, 6) as month_wid,
   ...
   left join bigdata_dm.pj_sl_in_store_day_a_tempb b
         on a.bu_wid = b.bg_wid;

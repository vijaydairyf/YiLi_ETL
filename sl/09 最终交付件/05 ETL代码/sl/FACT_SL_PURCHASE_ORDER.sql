...
from bigdata_dw.fact_sl_purchase_order_tmp tt1
left join (
select dealer_num,max(cp_factory_id) as cp_factory_id ,max(rp_factory_id) as rp_factory_id
from bigdata_ld.fact_cx_awk_dealer_factory_set group by dealer_num)  b on tt1.erp_dealer_code = b.dealer_num
left join  bigdata_dw.w_product_d cc on product_wid  = cc.row_wid;
...
from bigdata_dw.fact_sl_purchase_order_tmp tt1
  ...
  on request.integration_id = tt1.request_type_wid;

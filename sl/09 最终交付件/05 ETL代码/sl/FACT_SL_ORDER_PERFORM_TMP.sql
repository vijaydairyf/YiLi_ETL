...
insert overwrite table bigdata_dw.fact_sl_order_perform_tmp11
select header_id,
                    item_no,
               ...
               from (select header_id,
                       ...
                       from (
                             ...
                             select tt3.header_id, --订单头id
                               ...
                               from (select * from bigdata_ld.fact_sl_ld_erp_sales_order where date_ >= substr(regexp_replace(date_sub(current_date,90),'-',''),1,6))tt3
                              left join (select * from bigdata_ld.fact_sl_ld_dms_st_business where dt_wid >= date_sub(current_date,90))tt1
                                 on tt1.erp_line_id = tt3.line_id
                               left join bigdata_dw.w_bigdata_p tt4
                                 on tt1.bg_code = tt4.bg_code and tt4.name = '订单满足时限值'
              ...
              group by header_id, item_no, wh_code;
...
-- nvl(t4.header_id, 0) erp_header_id
  ...
  left join  bigdata_dw.fact_sl_order_perform_tmp11 t4

...
select cast(row_wid as bigint) as date_id,
       cast(regexp_replace(date_sub(regexp_replace(day_dt,'/','-'), 29),'-','') as bigint) as date_30_id
...
--------------------
insert overwrite table bigdata_dm.pj_sl_dealer_in_store_temp partition(date_)
...
select t.business_dt_wid,
  ...
  from (         select a1.business_dt_wid,
		  ...
		  where a1.business_dt_wid >= regexp_replace(add_months(current_date,-2),'-','')
        --月累计销售值
        union all
        select cast(d.date_id as bigint) as business_dt_wid,
         ...
         where substr(a1.business_dt_wid, 0, 6) = substr(d.date_id, 0, 6)
		   ...
		   and a1.business_dt_wid >= regexp_replace(add_months(current_date,-2),'-','')
         group by d.date_id,
                  a1.bg_wid,
                  a1.product_wid,
                  a1.area_wid,

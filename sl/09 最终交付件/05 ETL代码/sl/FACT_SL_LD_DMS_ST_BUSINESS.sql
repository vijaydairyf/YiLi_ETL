...
where a.partition_key>= regexp_replace(date_sub(current_date,10),'-','')
union all
select
a.business_id as row_id,
null as erpid,
...
inner join data_lake.d_pty_dealer_base b  on a.org_id = b.dealer_id and b.sale_type='Dealer'
inner join data_lake.d_prd_ys_product f on a.prod_id = f.prod_id and  f.src_table_name = 'dms_cx_awk_product'
where a.partition_key>= regexp_replace(date_sub(current_date,10),'-','')  and a.business_type in ('销售出库', '退货入库', '积分订单出库', '回货入库', '回单入库') and f.ou_id in( 'YN','YP')
;

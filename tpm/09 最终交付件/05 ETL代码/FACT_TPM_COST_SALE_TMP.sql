...
WHERE (division_id <> '1-15LXXP' or  division_id is null) and data_type_name in('执行','结案差异','预结案差异','支付差异-不可用','支付差异-可用','预结案实结案差异') and rep_month >= '2018-01'
GROUP BY division_name
    ...
    ,fin_process_mode
    ,system_name;
INSERT INTO TABLE bigdata_dw.fact_tpm_cost_sale_base 
SELECT division_name
    ,data_type_name
    ,rep_month
    --,case when division_name = '液态奶事业部' then nvl(exe_area_id,sales_area_id) else exe_area_id end exe_area_id
    --,case when division_name = '液态奶事业部' then nvl(exe_region_id,exe_area_id) else exe_region_id end exe_region_id
    ,nvl(exe_area_id,sales_area_id) exe_area_id
    ,CASE WHEN exe_region_id IS NULL AND max(budget_org_id) = '1-9FQOK7' THEN max(budget_org_id)
	  WHEN exe_region_id IS NULL THEN exe_area_id ELSE exe_region_id end  exe_region_id
    ,camp_type_name
    ,exp_subj_name
    ,function_type_name
    ,exp_from_name
    ...
    ,sum(amt) AS amt
FROM sdata_full.tpm_cx_xx_rep_base b
WHERE division_id = '1-15LXXP' and data_type_name in('执行','结案差异','预结案差异','支付差异-不可用','支付差异-可用','预结案实结案差异') and rep_month >= '2018-01'
GROUP BY division_name
    ,data_type_name
    ,rep_month
    ,exe_area_id
...
SELECT bg.row_wid AS bg_wid
    ...
    ,case when exe_region_id is null then area.big_area_name else region.big_area_name end as big_area_name--执行大区
    ,nvl(region.area_name,t4.name)--执行区域
    ,camp_type_name--活动类型
    ,exp_subj_name--末级科目
...
FROM (
    ...
    FROM bigdata_dw.fact_tpm_cost_sale_base b
...
insert overwrite table bigdata_dw.fact_tpm_cost_sale_tmp select * from bigdata_dw.fact_tpm_cost_sale_tmp where area_name is not null;

...
drop TABLE bigdata_dw.fact_tpm_cost_sale_base_tmp;
CREATE TABLE bigdata_dw.fact_tpm_cost_sale_base_tmp AS
SELECT division_name
    ,data_type_name
    ,rep_month
    --,case when division_name = '液态奶事业部' then nvl(exe_area_id,sales_area_id) else exe_area_id end exe_area_id
    --,case when division_name = '液态奶事业部' then nvl(exe_region_id,exe_area_id) else exe_region_id end exe_region_id
    ,nvl(exe_area_id,sales_area_id) exe_area_id
    ,nvl(exe_region_id,exe_area_id) exe_region_id
    ,camp_type_name
    ,exp_subj_name
    ,function_type_name
    ,exp_from_name
...
WHERE data_type_name in('执行','结案差异','预结案差异','支付差异-不可用','支付差异-可用','预结案实结案差异') and rep_month >= '2018-01'
...
SELECT bg.row_wid AS bg_wid
    ...
    ,nvl(region.area_name,t4.name) as area_name--执行区域
    ,camp_type_name--活动类型
    ,exp_subj_name--末级科目
...
FROM (
    ...
    LEFT JOIN bigdata_dw.fact_tpm_cost_sale_base_tmp b ON a.year_code = substr(nvl(b.rep_month, '8888-88'), 1, 4)
    ...
    FROM bigdata_dw.fact_tpm_cost_sale_base_tmp b
    WHERE nvl(b.rep_month,'1888-08')>='2018-11' and
        b.data_type_name in('结案差异','预结案差异','支付差异-不可用','支付差异-可用','预结案实结案差异')
    ) t
...
select diff.rep_month,
       ...
       case when exe_region_id is null then area.big_area_name else region.big_area_name end as big_area_name,
       nvl(region.area_name,t4.name) as area_name,
       ...
       data_type_name,
       case when division_name = '酸奶事业部' and data_type_name in ('执行','方案/签呈') then 
       case
             when substr(diff.rep_month,6,2)='01' then nvl(jan_implement_amt,'0')
             when substr(diff.rep_month,6,2)='02' then nvl(feb_implement_amt,'0')
             when substr(diff.rep_month,6,2)='03' then nvl(mar_implement_amt,'0')
             when substr(diff.rep_month,6,2)='04' then nvl(apr_implement_amt,'0')
             when substr(diff.rep_month,6,2)='05' then nvl(may_implement_amt,'0')
             when substr(diff.rep_month,6,2)='06' then nvl(june_implement_amt,'0')
             when substr(diff.rep_month,6,2)='07' then nvl(july_implement_amt,'0')
             when substr(diff.rep_month,6,2)='08' then nvl(aug_implement_amt,'0')
             when substr(diff.rep_month,6,2)='09' then nvl(sept_implement_amt,'0')
             when substr(diff.rep_month,6,2)='10' then nvl(oct_implement_amt,'0')
             when substr(diff.rep_month,6,2)='11' then nvl(nov_implement_amt,0)
             when substr(diff.rep_month,6,2)='12' then nvl(dec_implement_amt,'0')
        end else amt end as amt,
       apply_bud_num,
       apply_num,
       reduce_bud_flg,
       --diff.exe_area_name,
       ...
       from sdata_full.tpm_cx_xx_rep_base diff
        ...
        left join bigdata_dw.dm_tpm_area_d region on nvl(diff.exe_region_id,diff.exe_area_id) = region.tpm_region_id and region.div_type = 'region'
        left join bigdata_dw.dm_tpm_area_d area on diff.exe_area_id = area.tpm_region_id and area.div_type = 'area'
        left join sdata_full.tpm_s_org_ext T4 ON diff.sales_region_id = T4.ROW_ID
 --where diff.data_type_name in ('执行', '结案差异', '结案')
 where rep_month >= '2018-11' and (division_name in('液态奶事业部','冷饮事业部') or (division_name = '酸奶事业部' and data_type_name in ('执行','方案/签呈','结案')));
-----
...
select diff.rep_month,
       ...
       coalesce(sales_area_name,'缺省') sales_area_name,
       coalesce(sales_region_name,'缺省') sales_region_name,
       case when sales_region_id is null then area.big_area_name else region.big_area_name end as big_area_name,
       nvl(region.area_name,t4.name) as area_name,
       coalesce(accnt_id,'缺省') accnt_id,
       coalesce(accnt_name,'缺省') accnt_name,
       dealer_type as dealer_type,
       ...
       from sdata_full.tpm_cx_xx_rep_base diff
        left join bigdata_dw.dm_tpm_dealer_d  dealer ON dealer.tpm_dealer_id = diff.accnt_id
        left join bigdata_dw.dm_tpm_area_d region on nvl(diff.sales_region_id,diff.sales_area_id) = region.tpm_region_id and region.div_type = 'region'
        left join bigdata_dw.dm_tpm_area_d area on diff.sales_area_id = area.tpm_region_id and area.div_type = 'area'
        left join sdata_full.tpm_s_org_ext T4 ON diff.sales_region_id = T4.ROW_ID
 --where diff.data_type_name in ('执行', '结案差异', '结案')
 where rep_month >= '2018-11' and (division_name in('奶粉事业部','健康饮品事业部')  or (division_name = '酸奶事业部' and data_type_name not in ('执行','方案/签呈','结案'))); 
...
SELECT bg.row_wid as bu_wid
    ...
    ,sum(case when bg.row_wid = 7 then 0 else case when data_type_name in ('执行','方案/签呈') and diff.reduce_bud_flg = '是' then coalesce(diff.amt,0) else 0 end end) as approve_amt
...
where diff.big_area_name is not null and rep_month <= substr(current_date,1,7) and rep_month >= '2018-11'
...
SELECT bg_wid as bu_wid
    ...
    ,'缺省' as dealer_id
    ,dealer_type as dealer_type
    ,nvl(camp_num,'缺省') as camp_num
    ,nvl(camp_name,'缺省') as camp_name
    ,'缺省' as start_dt
    ,'缺省' as end_dt
